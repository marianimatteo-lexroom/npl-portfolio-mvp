...
export function StatusPill({ status }: { status: RowStatus }) {
  ...
  return (
    <span
      title={s.label}
      className={`inline-flex max-w-full items-center rounded-sm border px-1.5 py-0.5 text-[10px] leading-tight font-medium ${s.cls}`}
    >
      {s.label}
    </span>
  );
...
export function FieldInput({
  ...
  return (
    <input
      ...
      className={`w-full min-w-0 max-w-full truncate rounded-sm border-b bg-transparent px-0.5 py-0.5 text-xs outline-none transition-colors ${
